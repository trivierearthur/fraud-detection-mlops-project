...
def sample_raw_transaction_data():
    ...
    return pd.DataFrame({
        "trans_date_trans_time": ["01-01-2025 12:30", "02-01-2025 14:45", "03-01-2025 09:15"],
...
def sample_high_cardinality_data():
    ...
    return pd.DataFrame({
        "trans_date_trans_time": pd.date_range("2025-01-01", periods=n_rows, freq="h").strftime("%d-%m-%Y %H:%M"),
        "dob": pd.date_range("1960-01-01", periods=n_rows, freq="D").strftime("%d-%m-%Y"),
        ...
        "merchant": merchants,
        "category": np.random.choice(["grocery_pos", "gas_transport", "shopping_net"], n_rows),
...
def test_feature_engineering_creates_expected_features(sample_raw_transaction_data, capsys):
    """
    ...
    assert "trans_hour" in x.columns, "trans_hour not created from trans_date_trans_time"
    assert "trans_dayofweek" in x.columns, "trans_dayofweek not created from trans_date_trans_time"
    assert "trans_month" in x.columns, "trans_month not created from trans_date_trans_time"
    assert "customer_age" in x.columns, "customer_age not created from DOB"
    assert "merchant_distance_km" in x.columns, "merchant_distance_km not created from coordinates"
    
    # Verify raw columns are dropped
    assert "trans_date_trans_time" not in x.columns, "trans_date_trans_time should be dropped"
    assert "dob" not in x.columns, "dob should be dropped"
    
    # Verify feature values are reasonable
    assert x["trans_hour"].min() >= 0 and x["trans_hour"].max() < 24, "trans_hour out of valid range [0, 23]"
    assert x["trans_dayofweek"].min() >= 0 and x["trans_dayofweek"].max() < 7, "trans_dayofweek out of valid range [0, 6]"
    assert x["trans_month"].min() >= 1 and x["trans_month"].max() <= 12, "trans_month out of valid range [1, 12]"
    assert x["customer_age"].min() >= 0 and x["customer_age"].max() <= 110, "customer_age out of reasonable range"
    assert x["merchant_distance_km"].min() >= 0, "merchant_distance_km should be non-negative"
    assert (x["merchant_distance_km"] < 50).all(), "All sample distances should be < 50 km (test data is geographically close)"
...
def test_high_cardinality_columns_removal(sample_high_cardinality_data, capsys):
    ...
    x, removed_cols = engineer_transaction_features(df, verbose=True, high_cardinality_threshold=200)
    
    # With threshold of 200, the low-cardinality sample columns should NOT be removed
    assert "merchant" in x.columns, "merchant (50 unique) should be kept (below threshold of 200)"
    ...
    assert "merchant" not in x_strict.columns, "merchant (50 unique) should be removed with threshold of 25"
    assert "job" not in x_strict.columns, "job (30 unique) should be removed with threshold of 25"
    assert "merchant" in removed_cols_strict, "merchant should be in removed_cols_strict list"
...
def test_transaction_feature_transformer_consistency(sample_raw_transaction_data):
    ...
    assert hasattr(transformer, "feature_columns_"), "Transformer should store feature_columns_ after fit"
    assert isinstance(transformer.feature_columns_, list), "feature_columns_ should be a list"
    ...
    assert list(x_train.columns) == transformer.feature_columns_, "Train transform should have fitted feature columns"
    ...
    assert list(x_test.columns) == transformer.feature_columns_, "Test transform should have same feature columns as train"
    assert len(x_test.columns) == len(x_train.columns), "Feature count should match between train and test transforms"
    
    # Verify all features are numeric or can be used in model training
    for col in x_test.columns:
        assert x_test[col].dtype in [np.float64, np.float32, np.int64, np.int32, object, "string"], \
